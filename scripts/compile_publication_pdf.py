...
<br>
• <strong>Multi-Objective Reinforcement Learning Co-Folding:</strong> We employ Proximal Policy Optimization (PPO) to steer generative sampling toward optimal pharmacological space, utilizing an aggregated reward function that explicitly enforces chemical validity, synthesizability (SA), quantitative drug-likeness (QED), Lipinski compliance, and binding affinity (pK_pred).
<br>
...
<h2>3.4 Multi-Objective Reinforcement Learning Co-Folding Formulation</h2>
<p>
...
<div class="math-block">
    R(m) = w_1 * QED(m) + w_2 * pK_pred(m) - w_3 * SA(m) + w_4 * I(Lipinski(m))
</div>
<p>
