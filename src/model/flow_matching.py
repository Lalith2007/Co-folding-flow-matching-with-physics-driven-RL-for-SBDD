...
class FlowMatching(nn.Module):
    ...
    def sample(
        ...
        # Start from pure noise — store RNG state for RL reproducible re-runs
        ...
        if element_bias is not None:
            z_type = z_type + element_bias.to(device)

        # Use softmax with temperature for smooth categorical exploration across all elements (C, N, O, S, F, Cl)
        if temperature <= 0.05:
            atom_types = z_type.argmax(dim=-1)
            type_probs = F.softmax(z_type, dim=-1)
        else:
            type_probs = F.softmax(z_type / max(temperature, 0.1), dim=-1)
        ...
        res = {
            ...
            "z_type_final": z_type,       # Raw pre-decode logits — used by RL for log_prob
            "z_coord_final": z_coord,     # Final coords for RL re-run anchor
            "noise_rng_state": rng_state, # RNG state at start — for reproducible RL re-run
            "pK_pred": pK_pred,
            "num_atoms": N_L,
